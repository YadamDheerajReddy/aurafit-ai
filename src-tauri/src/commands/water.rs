...
#[tauri::command]
pub async fn log_water(pool: State<'_, SqlitePool>, input: LogWaterInput) -> Result<i64, String> {
    if input.amount_ml <= 0 {
        return Err("Amount must be positive.".to_string());
    }

    let id: i64 = sqlx::query_scalar(
        "INSERT INTO water_log (amount_ml) VALUES (?) RETURNING id",
    )
...
#[tauri::command]
pub async fn get_todays_water(pool: State<'_, SqlitePool>) -> Result<TodaysWater, String> {
    let entries = sqlx::query_as::<_, WaterLogRow>(
        "SELECT id, amount_ml, logged_at FROM water_log
         WHERE date(logged_at) = date('now') ORDER BY logged_at DESC, id DESC",
    )
...
#[tauri::command]
pub async fn delete_water_entry(pool: State<'_, SqlitePool>, id: i64) -> Result<(), String> {
    sqlx::query("DELETE FROM water_log WHERE id = ?")
        .bind(id)
        .execute(pool.inner())
        .await
        .map_err(|e| e.to_string())?;
